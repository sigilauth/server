...
//
// Algorithm per wire-protocol.md §2.3 (SIGIL-CONV-V1):
...
// 4. encryption_key = HKDF(ikm=shared_secret, salt=recipient_fingerprint, info=context, length=32)
...
// - fingerprint: Recipient's key fingerprint (used as HKDF salt)
// - context: HKDF info string (e.g., "SIGIL-CONV-V1-AES256", "sigil-decrypt-v1")
//
...
func Encrypt(recipientPublicKey *ecdsa.PublicKey, plaintext, fingerprint []byte, context string) ([]byte, error) {
	...
	encryptionKey, err := DeriveKey(sharedX.Bytes(), fingerprint, context, 32)
	if err != nil {
