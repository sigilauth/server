...
func main() {
	...
	// Configure base URL for QR codes (from environment)
	if baseURL := os.Getenv("SIGIL_BASE_URL"); baseURL != "" {
		h.SetBaseURL(baseURL)
		log.Printf("QR base URL configured: %s", baseURL)
	} else {
		log.Println("SIGIL_BASE_URL not set - using fallback https://sigil.example.com for QR codes")
	}
